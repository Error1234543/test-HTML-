# bot.py (Updated with OCR fallback for copyable text)
import os
import json
import time
import logging
...
GEMINI_API_KEY = os.getenv("AIzaSyB5TA6nDIj8VARsC4LPfdxu7_HBnetmPg8") or ""
...
def choose_model():
    try:
        ...
        logging.warning("list_models failed: %s — fallback to %s", e, MODEL_CANDIDATES[0])
        return MODEL_CANDIDATES[0]
...
console.log(questions); // copyable text
</script>
</body>
</html>"""
...
def webhook():
    ...
    try:
        if "message" in update and "document" in update["message"]:
            ...
            send_message(chat_id, "PDF received — extracting text...")
            ...
            if not text.strip():
                send_message(chat_id, "PDF seems scanned — using OCR...")
                pages = convert_from_path(local_pdf)
                for page in pages:
                    text += pytesseract.image_to_string(page, lang='eng+guj+hin') + "\n"

            send_message(chat_id, "Parsing MCQs from PDF...")
            parsed = parse_mcqs_from_text(text)
            if not parsed:
                send_message(chat_id, "No MCQs detected automatically. Try clearer PDF.")
                return jsonify(ok=True)

            send_message(chat_id, f"Detected {len(parsed)} questions. Finding answers via Gemini...")
            ...
            send_document(chat_id, out_path, caption="Interactive HTML test — open in Chrome.")
            send_message(chat_id, "Done — HTML test sent.")
        else:
            if "message" in update and "text" in update["message"]:
                chat_id = update["message"]["chat"]["id"]
                txt = update["message"]["text"].strip().lower()
                if txt in ("/start","start"):
                    send_message(chat_id, "Send a PDF (Gujarati/Hindi/English). I'll extract MCQs, detect answers, and return copyable HTML test.")
                else:
